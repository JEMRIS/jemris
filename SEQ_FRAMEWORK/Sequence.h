...
public:
 Sequence(){ m_bNewParam=false; m_dFactor=1.0; m_iTreeSteps=0; };
 ~Sequence(){};

  //virtual functions to be implemented by derived classes
  virtual double getDuration()=0;
  virtual bool  getValue(const double time , double* const d_AllValuesPtr)=0;
  virtual void Destroy()=0;
  virtual void writeADCs(ofstream* pfout)=0;
  virtual void writeSeqVal(double& dTimeShift,ofstream* pfout)=0;
  ...
  Sequence* getRoot ( ) {Sequence* S=this; for(;;){S=S->getParent(); if (S==S->getParent()) return S;} }; 
  string getName ( ) {return m_sName;};
  void setName (string value ) {m_sName=value;};

  void setParent(Sequence* value ){m_cParentPtr = value;};
  ...
  //these functions might be used during Prepare() to check whether parameters have changed
