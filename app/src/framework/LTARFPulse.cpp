...
/***********************************************************/
bool LTARFPulse::Prepare  (PrepareMode mode) {
...
/***********************************************************/
double            LTARFPulse::GetMagnitude (double const time)  {

	double magn = 0.0;

	magn = m_Gamma * m_Alpha * m_A / GetDuration() * exp(-pow(m_Beta*time/GetDuration(),2)) *
		sqrt( pow(2*PI*m_N*time/GetDuration(),2) + 1);
