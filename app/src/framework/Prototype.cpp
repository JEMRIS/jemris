...
/***********************************************************/
bool Prototype::Notify (void* ppriv, bool record){
	...
	for(iaddr = m_attrib_addr.begin(); iaddr != m_attrib_addr.end(); iaddr++)
		if (iaddr->second == ppriv)
			Notify(iaddr->first, record);

	return true;
};

/***********************************************************/
void Prototype::Notify (string attrib, bool record) {
	...
	for (it=itp.first; it!=itp.second; ++it) {
		...
		//set this attribute to be dynamic, if it is not already
		if (record)
			if (!obs.first->IsDynamic(obs.second))
				 obs.first->SetDynamic(obs.second);

		//Calculate linked-attributes of the observer
		(obs.first)->EvalExpressions(obs.second,PREP_UPDATE,record);
...
/***********************************************************/
void    Prototype::EvalExpressions (string attrib,PrepareMode mode, bool record) {
	...
	if (record){
		//cout << "!!!!" << d << endl;
		if (t==typeid(double*).name()  ) m_record_double.insert(pair< string , double >(attrib,((double) d )));
		if (t==typeid(int   *).name()  ) m_record_int.insert(   pair< string , int    >(attrib,((int   ) d )));
	}
	...
	Notify(p,record); //Notify other modules about change of this attribute
