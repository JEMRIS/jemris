...
class      Sequence {
 ...
 public:
	...
	virtual bool        getValue            (const double time, 
                                             double* const d_AllValuesPtr) = 0;
	
	virtual void        Destroy             ()=0;
	
	virtual void        writeADCs           (ofstream* pfout) = 0;
	
	virtual void        writeSeqVal         (double& dTimeShift,
                                             ofstream* pfout) = 0;
	
	virtual int         getNumberOfChildren ()
        {return 0;};
	
	virtual Sequence*   getChild            (int ChildID)
        {return NULL;};
	
	virtual bool        Prepare             (bool verbose)
        {return true;};
