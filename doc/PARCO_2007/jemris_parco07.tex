...
\\
The {\it Sequence} design of the sequence class and its underlying
...
parts, loops (branch nodes) and modules (leaf nodes) from an abstract
module class. To reduce the complexity in and
promote the encapsulation of the objects in this part of the framework
a prototypical approach is implemented as suggested by
Gamma et al. 
\\
A sequence (object) is populated via an XML file during startup of the
simulator to represent a left-right ordered
tree. Fig.~\ref{fig:epitree} depicts by example how the different
modules of an EPI sequence, loops (sequences) and pulses, can be arranged in an
...
for the pulses and, thus, beare the 
the functionality to emit pulses of various types. The pulses
themselves are defined in a seperate class hierarchy. 
\\
JEMRIS uses an XML representation of the suggested sequence tree, i.e.
sequences can be read from or written to XML files. The C++ Xerces XML
parser provided by the Apache Software Foundation\footnote{\sf
