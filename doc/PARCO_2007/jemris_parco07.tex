...
In the field of pulse sequence optimisation, e.g.~for artefact detection and elimination, simulations allow one to
differentiate between effects related to physics and hardware imperfection. Further, if the simulation environment
is able to simulate the hardware malfunction, then results may be used for the optimisation of the hardware
...
Another important direction of application is image generation for the purpose of image processing -- here, complete
control of the properties of the input data allows a tailored design of image processing algorithms for certain applications.\\
\ \\
...
spin ensemble needs to be simulated in order to obtain realistic results. As such, several published approaches have reduced the
size of the problem in different ways. The most prominent method is to consider only cases in which analytical solutions to the
problem exist \cite{benoit}. However, for the case of radiofrequency (RF) excitation in the presence of time varying gradient fields no
analytical solution exists and, thus, the important field of selective excitation cannot be studied with such an approach.
Apart from the computational demand, the complexity of the MRI imaging sequence is an additional obstacle. The difficulty of MRI
...
The JEMRIS simulator is based on a classical description of MRI physics by means of the  Bloch equations, describing the sample by its
...
an external magnetic field. For MRI, the field decomposes to the strong static component, $B_0$, a temporally and spatially varying field along
the same direction (the imaging gradients ${\bf G}$) and the orthogonal components of the RF excitation field, $B_1$.
...
\end{small}
\ \\
where $\gamma$ is the gyromagnetic ratio of the nuclei (usually protons) under consideration.
The complex MR signal is then described by the signal equation
...
evolution of each spin isochromate is completely other and the problem is ideally suited for numerical treatment with
...
in various coils to produce the RF field for excitation and the gradient field for encoding spatial information to the phase/frequency of the MR
signal: 
...
of {\it in vivo} spectroscopy or for the challenging task of correcting subject-induced $B_1$ field inhomogeneities at ultra high fields \cite{ibrahim}.
However, thus far the computation of these pulses is based on a simplified physical model neglecting relaxation and
off-resonance effects during the pulse. Under such conditions, the calculation of the unknown RF pulse shape, $B_1$, from a
...
\end{equation}
where $M_0$ is the equilibrium magnetisation and ${\bf k}(t)$ is a given k-space trajectory.
...
methods.
In contrast, the present approach provides a numerical method to design selective pulses under realistic experimental
...
with respect to the real and the imaginary parts of $B_1= B_{1x} + \mbox{i}B_{1y}$. The starting point ($B_{1x},B_{1y}$) for each of
...
premises: Obviously, the object design had to reflect the physical
...
realised with appropriate MPI data-types. Similarly, the {\it Signal}
holds information about the `MR signal' consisting of the net
...
as for the {\it Sample} class. The {\it Model} class contains the
...
the most complex part of the simulator. For this a novel object-oriented
design pattern for MRI is introduced which derives the basic
...
modules of the well-known EPI sequence \cite{haacke}, loops and pulses, can be arranged in an
...
MATLAB\footnote{\sf http://www.mathworks.com} which allows one to
...
\epsfiguresv{fig/EPI_sketch.eps}{fig//EPI_tree.eps}{{\bf Top:} Sketch of a native EPI pulse sequence diagram,
		...
		in which other modules are concatenated. Leaf nodes represent the modules in which the actual pulse shapes are
...
Pseudo code of the parallel workflow of JEMRIS is shown in the parallel algorithm \ref{parcode1}. (Comments
are written with a reduced font size.) The (mostly sleeping) master process subdivides the problem
in the beginning and harvests the result at the end. The basic functionality of the slave processes is hidden
in the {\it Solve()} method of the {\it Model} class, where the solution for each spin is obtained during a recursive
travel through the sequence tree. All functionality for introducing off-resonance effects is hidden in 
...
%%%
The computation of selective excitation pulses utilising the simulator is shown in the parallel algorithm \ref{parcode2}.
...
to perform 2D MRI simulations within minutes. Thus, the simulations presented here reduce to 2D examples, though the simulator
...
amount of time, 28 \%, is spent in the {\it Sequence.getValue()} function, leaving room for future optimisation of data retrieval from
the sequence tree. As expected, parallel performance scales nearly perfectly, i.e.~the speed increases linearly with the number
of processors.
%%%%%%%%%%%%%%%%
...
\epsfigures{fig/GUI_seq_ex1.eps}{fig/GUI_sim_ex1.eps}{{\bf Left:}  Example of the JEMRIS GUI for sequence development showing the EPI sequence
			     already shown in Fig.~\ref{fig:epitree}. In the top left, the sequence is interactively built and on the right
			     ...
			     ( 60$^\circ$-90$^\circ$-90$^\circ$ at 0-10-25 ms) applied to a homogeneous sphere under strong field inhomogeneities
			     results in the well-established five echoes: three spin echoes, one reflected spin echo, and the stimulated
...
%%%%%%%%%%%%
Fig.~\ref{fig:gui} shows an example of the GUI for sequence development, which allows interactive
...
well-established five MR echoes generated by such a three pulse experiment are visible. The tracking of magnetisation can be accurately
...
yield the prominent fat-water shift in MRI; b) malfunctioning MR scanner hardware simulation with a nonlinear gradient field results
in a distorted EPI image; c) TrueFISP simulation including susceptibility-induced field inhomogeneity yields the well-known
...
Fig.~\ref{fig:artifacts} d), cannot be realised with any simulator relying on analytical solutions of the Bloch
equation due to neglect of relaxation effects during the RF pulse. The exact simulation of the simultaneous occurrence of RF
pulses and imaging gradients with JEMRIS is the foundation for the derivation of new selective excitation pulses presented in the next section.
\begin{small}
 \begin{table}[tp]
  {\footnotesize
  \begin{center}
...
\epsfigure{fig/Selex.eps}{Example of multidimensional excitation. {\bf Left Column:} 2D MR sample and the target pattern for selective
			     excitation. {\bf Middle Column:} RF pulses computed with the common approach and with the proposed method
...
computed with the new approach shows fewer higher spatial frequency components than the common
approach. This is a well-known problem of the common linear approach, leading to high frequency artefacts on
selectively excited images, as can be seen from the excited target pattern in Fig.~\ref{fig:selex}. In comparison, the new approach
excites a pattern that better approximates the target pattern.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusion}
...
variety of experimental conditions. The parallel HPC implementation allows for the treatment of huge spin ensembles
resulting in realistic MR signals. On small HPC clusters, 2D simulations can be obtained on the order of minutes,
