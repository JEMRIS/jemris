...
image (MRI) generation, e.g. for general pulse sequence design and optimization, artifact detection, validation of
...
of the excitation process. In general, the interpretation and validation of experimental results benefits from comparisons
...
of in vivo spectroscopy or for the challenging task of correcting subject-induced $B_1$ field inhomogeneities at ultra high field \cite{ibrahim}.
...
%%%%%%%%%%%%%
\epsfigure{fig/classes.eps}{Class hierarchy of the basic JEMRIS components, Sample, Signal, Model, and Sequence class,
...
Message passing is realised with the C++ bindings of the MPI library \footnote{The MPI standard. {\sf http://www-unix.mcs.anl.gov/mpi/}}.
...
have the functionality to set pulses of various types. The pulses itself are defined in a separate class hierarchy.
JEMRIS uses XML to store the sequence tree information, i.e. sequences can be read from XML files, where the C++ Xerces XML parser
from Apache is used\footnote{\sf http://xml.apache.org/xerces-c/}. JEMRIS parses the XML file and automatically generates the sequence
tree object. Further, a GUI was implemented in MATLAB\footnote{\sf http://www.mathworks.com} which allows to interactively build
the sequence tree, view the pulse diagram, and to perform the simulation. For this, the GUI calls JEMRIS via {\sf ssh} on the HPC cluster.
...
Pseudo code of the parallel work flow of JEMRIS is shown in the parallel algorithm diagram \ref{parcode1}. (Comments
are written with a reduced fontsize.) The (mostly sleeping) master process subdivides the problem
in the beginning and collects the result in the end.  The basic functionality of the slave processes is hidden
...
to perform 2D MRI simulations within minutes. Thus, the here presented simulations reduce to 2D examples, also the simulator
is able to treat 3D simulations. The performance of the simulator is depicted in Fig.~\ref{fig:performance}. The results from
...
the sequence tree. As expected, the parallel performance scales nearly perfectly, i.e.~the speed linearly increases with the number
of processors.
%%%%%%%%%%%%%%%%
\epsfigure{fig/performance.eps}{JEMRIS performance testing. {\bf Left:} Sequential profiling shows that $\approx$ 70 \% of the time
                             is spent in the highly optimised CVODE library. {\bf Right:} The speed
			     gain due to parallelisation (right) is close to optimal for large scale problems, ax expected.}
...
\epsfigures{fig/GUI_seq_ex1.eps}{fig/GUI_sim_ex1.eps}{{\bf Left:}  Example of the JEMRIS GUI for sequence development: the EPI sequence
			     ...
			     results in the famous five echoes: three spin echos, one reflected spin echo, and the stimulated
