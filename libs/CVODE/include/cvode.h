...
typedef void (*RhsFn)(integer N, cvreal t, N_Vector y, N_Vector ydot,
                      void *f_data);
 ...
 *            later) that will hold optional cvreal inputs and      *
 *            outputs.  The user can pass NULL if he/she does not *
 *            wish to use optional cvreal inputs or outputs.        *
 ...
 *          errors, yout=y(tout).                                 *
 *                                                                *
 * t     is a pointer to a cvreal location. CVode sets (*t) to the  *
 ...
 *                 Optional output.                               *
 *                                                                *
 * iopt[LENRW]   : size of required CVODE internal cvreal work      *
 *                 space, in cvreal words.  Optional output.        *
 *                                                                *
 ...
 * the number of integer and cvreal optional inputs and outputs     *
...
typedef struct CVodeMemRec {
  ...
  cvreal cv_uround;    /* machine unit roundoff */
  ...
  cvreal *cv_reltol;     /* ptr to relative tolerance   */
  ...
  cvreal cv_h;        /* current step size                     */
  cvreal cv_hprime;   /* step size to be used on the next step */ 
  cvreal cv_eta;      /* eta = hprime / h                      */
  cvreal cv_hscale;   /* value of h used in zn                 */
  cvreal cv_tn;       /* current internal value of t           */

  cvreal cv_tau[L_MAX+1];    /* array of previous q+1 successful step     */
                           /* sizes indexed from 1 to q+1               */
  cvreal cv_tq[NUM_TESTS+1]; /* array of test quantities indexed from     */
                           /* 1 to NUM_TESTS(=5)                        */
  cvreal cv_l[L_MAX];        /* coefficients of l(x) (degree q poly)      */

  cvreal cv_rl1;      /* 1 / l[1]                     */
  cvreal cv_gamma;    /* gamma = h * rl1              */
  cvreal cv_gammap;   /* gamma at the last setup call */
  cvreal cv_gamrat;   /* gamma / gammap               */

  cvreal cv_crate;   /* estimated corrector convergence rate */
  cvreal cv_acnrm;   /* | acor | wrms                        */
  ...
  cvreal cv_hmin;     /* |h| >= hmin       */
  cvreal cv_hmax_inv; /* |h| <= 1/hmax_inv */
  cvreal cv_etamax;   /* eta <= etamax     */
  ...
  cvreal cv_hu;           /* last successful h value used   */
  cvreal cv_saved_tq5;    /* saved value of tq[5]           */
  boole cv_jcur;        /* Is the Jacobian info used by   */
                        /* linear solver current?         */
  cvreal cv_tolsf;        /* tolerance scale factor         */
  ...
  cvreal     *cv_ropt;  /* real optional input, output     */
