...
cvreal **bandalloc(integer n, integer smu, integer ml)
{
  ...
  a = (cvreal **) malloc(n * sizeof(cvreal *));
  if (a == NULL) return(NULL);

  colSize = smu + ml + 1;
  a[0] = (cvreal *) malloc(n * colSize * sizeof(cvreal));
  if (a[0] == NULL) {
    free(a);
    return(NULL);
  }
             ...
             integer *p)
{
  ...
  cvreal *a_c, *col_k, *diag_k, *sub_diag_k, *col_j, *kptr, *jptr;
  cvreal max, temp, mult, a_kj;
  boole swap;
...
void gbsl(cvreal **a, integer n, integer smu, integer ml, integer *p,
	  cvreal *b)
{
