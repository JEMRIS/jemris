...
static boole CVEwtSet(CVodeMem cv_mem, cvreal *rtol, void *atol, int tol_type,
                      N_Vector ycur, integer neq);
static boole CVEwtSetSS(CVodeMem cv_mem, cvreal *rtol, cvreal *atol,
                        N_Vector ycur, integer neq);
static boole CVEwtSetSV(CVodeMem cv_mem, cvreal *rtol, N_Vector atol,
                        N_Vector ycur, integer neq);

static boole CVHin(CVodeMem cv_mem, cvreal tout);
static cvreal CVUpperBoundH0(CVodeMem cv_mem, cvreal tdist);
static cvreal CVYddNorm(CVodeMem cv_mem, cvreal hg);
...
static cvreal CVAdamsStart(CVodeMem cv_mem, cvreal m[]);
static void CVAdamsFinish(CVodeMem cv_mem, cvreal m[], cvreal M[], cvreal hsum);
static cvreal CVAltSum(int iend, cvreal a[], int k);
static void CVSetBDF(CVodeMem cv_mem);
static void CVSetTqBDF(CVodeMem cv_mem, cvreal hsum, cvreal alpha0,
		       cvreal alpha0_hat, cvreal xi_inv, cvreal xistar_inv);
...
static int  CVHandleNFlag(CVodeMem cv_mem, int *nflagPtr, cvreal saved_t,
			  int *ncfPtr);

static void CVRestore(CVodeMem cv_mem, cvreal saved_t);

static boole CVDoErrorTest(CVodeMem cv_mem, int *nflagPtr, int *kflagPtr,
                           cvreal saved_t, int *nefPtr, cvreal *dsmPtr);

static void CVCompleteStep(CVodeMem cv_mem);

static void CVPrepareNextStep(CVodeMem cv_mem, cvreal dsm);
static void CVSetEta(CVodeMem cv_mem);
static cvreal CVComputeEtaqm1(CVodeMem cv_mem);
static cvreal CVComputeEtaqp1(CVodeMem cv_mem);
static void CVChooseEta(CVodeMem cv_mem,cvreal etaqm1, cvreal etaq, cvreal etaqp1);
...
void *CVodeMalloc(integer N, RhsFn f, cvreal t0, N_Vector y0, int lmm, int iter,
		  int itol, cvreal *reltol, void *abstol, void *f_data,
		  FILE *errfp, boole optIn, long int iopt[], cvreal ropt[],
		  void *machEnv)
{
...
int CVReInit(void *cvode_mem, RhsFn f, cvreal t0, N_Vector y0,
             int lmm, int iter, int itol, cvreal *reltol, void *abstol,
             void *f_data, FILE *errfp, boole optIn, long int iopt[],
             cvreal ropt[], void *machEnv)
{
  boole   allocOK, ioptExists, roptExists, neg_abstol, ewtsetOK;
  int     maxord;
  CVodeMem cv_mem;
...
int CVode(void *cvode_mem, cvreal tout, N_Vector yout, cvreal *t, int itask)
{
...
int CVodeDky(void *cvode_mem, cvreal t, int k, N_Vector dky)
{
  cvreal s, c, r;
  cvreal tfuzz, tp, tn1;
  int i, j;
...
/****************** CVAllocVectors ***********************************
 ...
 (respectively) the lengths of the cvreal and integer work spaces
 allocated here.
...
/*********************** CVEwtSet **************************************
 ...
 (2) ewt[i] = 1 / (*rtol * ABS(ycur[i]) + atol[i]), i=0,...,neq-1
  ...
  All the cvreal work is done in the routines CVEwtSetSS, CVEwtSetSV.
...
{
  switch(tol_type) {
  case SS: return(CVEwtSetSS(cv_mem, rtol, (cvreal *)atol, ycur, neq));
...
{
  int sign, count;
  cvreal tdiff, tdist, tround, hlb, hub;
  cvreal hg, hgs, hnew, hrat, h0, yddnrm;
...
static cvreal CVUpperBoundH0(CVodeMem cv_mem, cvreal tdist)
{
  cvreal atoli, hub_inv, hub;
  boole vectorAtol;
  N_Vector temp1, temp2;

  vectorAtol = (itol == SV);
  if (!vectorAtol) atoli = *((cvreal *) abstol);
  temp1 = tempv;
  temp2 = acor;
  N_VAbs(zn[0], temp1);
  N_VAbs(zn[1], temp2);
...
static void CVIncreaseBDF(CVodeMem cv_mem)
{
  cvreal alpha0, alpha1, prod, xi, xiold, hsum, A1;
  int i, j;
...
static void CVSetBDF(CVodeMem cv_mem)
{
  cvreal alpha0, alpha0_hat, xi_inv, xistar_inv, hsum;
  int i,j;
...
static void CVSetTqBDF(CVodeMem cv_mem, cvreal hsum, cvreal alpha0,
		       cvreal alpha0_hat, cvreal xi_inv, cvreal xistar_inv)
{
  cvreal A1, A2, A3, A4, A5, A6;
  cvreal C, CPrime, CPrimePrime;
