...
SpgmrMem SpgmrMalloc(integer N, int l_max, void *machEnv)
{
  ...
  Hes = (cvreal **) malloc((l_max+1)*sizeof(cvreal *)); 
  if (Hes == NULL) {
  ...
  for (k = 0; k <= l_max; k++) {
    Hes[k] = (cvreal *) malloc(l_max*sizeof(cvreal));
    if (Hes[k] == NULL) {
  ...
  givens = (cvreal *) malloc(2*l_max*sizeof(cvreal));
  if (givens == NULL) {
  ...
  yg = (cvreal *) malloc((l_max+1)*sizeof(cvreal));
  if (yg == NULL) {
    N_VFree(xcor);
    free(givens);
    for (i = 0; i <= l_max; i++) free(Hes[i]);
...
int SpgmrSolve(SpgmrMem mem, void *A_data, N_Vector x, N_Vector b,
               int pretype, int gstype, cvreal delta, int max_restarts,
	       void *P_data, N_Vector s1, N_Vector s2, ATimesFn atimes,
	       PSolveFn psolve, cvreal *res_norm, int *nli, int *nps)
{
  N_Vector *V, xcor, vtemp;
  cvreal **Hes, *givens, *yg;
  cvreal beta, rotation_product, r_norm, s_product, rho;
