function varargout = JEMRIS_seq2(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @JEMRIS_seq2_OpeningFcn, ...
                   'gui_OutputFcn',  @JEMRIS_seq2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before JEMRIS_seq2 is made visible.
function JEMRIS_seq2_OpeningFcn(hObject, eventdata, handles, varargin)
colordef white

% Choose default command line output for JEMRIS_seq2
...
hax{1}=handles.axes1; hax{2}=handles.axes2; hax{3}=handles.axes3;
hax{4}=handles.axes4; hax{5}=handles.axes5; hax{6}=handles.axes6;
for i=1:6;set(hax{i},'color',[1 1 1],'visible','off');end
handles.hax=hax;
handles.dm=0;
handles.ax=0;
handles.CWD=pwd;
handles.JemrisPath=fileparts(which('JEMRIS_seq2'));

%all modules + attributes which are currently supported by JEMRIS
handles.Modules={'ConcatSequence','AtomicSequence','DelayAtom','EmptyPulse','HardRfPulseShape',...
                 'TGPS','RO_TGPS','PE_TGPS','RfPhaseCycling','RfSpoiling'};
handles.Attributes={...
        ...
        {'Name','Axis','Area','FlatTop','ADCs','Factor','Gmax','SlewRate'};...
...
handles.Parameter={'TR','TE','TI','TD','Nx','Ny','FOVx','FOVy','ReadBW'};
...
function varargout = JEMRIS_seq2_OutputFcn(hObject, eventdata, handles) 
varargout{1} = handles;
...
handles.seqfile=FileName;
handles.Seq=S;
hax=handles.hax;for i=1:5; cla(hax{i},'reset'); set(hax{i},'visible','off'); end
guidata(hObject, handles);
update_Callback(hObject, eventdata, handles);
...
axes(handles.hax{6}); cla(handles.hax{6},'reset');
