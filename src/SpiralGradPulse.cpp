...
double            SpiralGradPulse::GetGradient (double const time)  {
	...
	return (m_inward) ? -val : val;  ; // WIP: Amplituden für Spiral In drehen
...
bool              SpiralGradPulse::Prepare     (const PrepareMode mode)   {
    ...
    if ( mode == PREP_VERBOSE) {
        ...
        if (!HasDOMattribute("MaxSlew"))    {
            btag = false;
            cout << GetName() << "::Prepare() error: MaxSlew [mT/m/ms] required.\n";
			return false;
        }

        if (!HasDOMattribute("MaxGrad"))    {
            btag = false;
            cout << GetName() << "::Prepare() error: MaxGrad [mT/m] required.\n";
			return false;
        }
		...
		if (!HasDOMattribute("GradRasterTime"))    {
            btag = false;
            cout << GetName() << ":Warning: GradRasterTime [ms] not defined. Set to default of 0.01 ms.\n";
			// m_grad_raster_time = 0.01;
        }
	...
	if (mode == PREP_VERBOSE) {
		...
		double Gmax = m_max_grad/10.;		// mT/m -> G/cm
		double Smax = m_slewrate/10.;		// mT/m/ms -> G/cm/ms
		m_fov /= 10.;						// [mm] >> [cm]
