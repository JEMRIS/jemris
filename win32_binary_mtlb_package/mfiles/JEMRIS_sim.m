...
sample.R=50;sample.DxDy=3; sample.gamBo=63.87;
...
set(handles.JobTag,'Visible','off');
...
function handles=write_simu_xml(handles,redraw)
 ...
 if isempty(handles.UserSample)
    ...
    writeSample(sample.name,round(2*R/res),res,0,M0,T1,T2,CS,0,sample.file,handles.Processes);
 else
    handles.UserSample.Processes = handles.Processes;
    writeSample(handles.UserSample);
 end       
 %RX coilarray
 ...
 NAMES = {'localFieldFluctuations','EvolutionSteps','EvolutionPrefix','ConcomitantFields'};
 VALUES= {num2str(sim.DF),num2str(sim.INC),'evol',num2str(sim.CF)};
 ...
 SIMU.Children(3)=TXCA; SIMU.Children(4)=PARA; SIMU.Children(5)=SEQU; SIMU.Children(6)=MODE;
 %write simu file(s), only if sequence exists!
 if (exist(fullfile(handles.seqdir,handles.seqfile)) == 2)
  ...
  for j=1:P
   ...
   if P>1
     ...
     sname = sprintf('%d%s',j,sname);
   end
   xname = sprintf('%d%s',j,xname);
   RXCA.Attributes(2).Name='SignalPrefix'; RXCA.Attributes(2).Value=sname;
   SAMP.Attributes(2).Value=fullfile(cwd,fname);
   SIMU.Children(1)=SAMP; SIMU.Children(2)=RXCA;  
   writeXMLseq(handles,SIMU,xname);
  end
 else
...
function Sample_Callback(hObject, eventdata, handles)
 ...
 switch Nsample
    case 1 %sphere
        ...
        handles.sample.M0=1;handles.sample.R=50;handles.sample.DxDy=3;
    ...
    case 2 %2spheres
        ...
        handles.sample.M0=[1 1];handles.sample.R=[50 25];handles.sample.DxDy=3;
        handles.sim.DF=0;handles.sim.CSF=1;
        M0str='M0'; CSstr='CS [Hz]'; SSstr='Radius'; T1str='T1 [ms]';  T2str='T2 [ms]';
   case 3 %brain
        bv1='off';bv2='on';
        ...
        handles.sample.M0=1;handles.sample.R=90;handles.sample.DxDy=3;
...
function start_simu_Callback(hObject, eventdata, handles)
 ...
 P=handles.Processes;
 for j=1:P
  COMMAND = sprintf('%s/jemris %dsimu.xml > .%dsim.out &',handles.JemrisPath,j,j);
...
pause(1)
 ...
 Ca={};
 for j=1:P
  if P>1,Ca=[Ca,{''},{sprintf('Job #%d',j)}];end
  while 1
   C={};
   fid=fopen(sprintf('.%dsim.out',j));
   while 1
     tline = fgetl(fid);
 ...
 set(handles.sim_dump,'String',Ca,'FontName','monospaced','FontSize',8);
 pause(1)
...
[dummy,handles.txaname]=fileparts(FileName);
set(handles.RxText,'String',['Rx-Array: ',FileName]);
guidata(hObject, handles);
...
handles.sim.DF=str2num(get(hObject,'String'));
guidata(hObject, handles);
...
function saveMagEvolTag_Callback(hObject, eventdata, handles)
 [FileName,PathName] = uiputfile('*.mat');
 if FileName==0,return;end
 [M,x,y,t]=readEvol(pwd);
 save(FileName,'t','M','x','y');
 



function JobTag_Callback(hObject, eventdata, handles)
p=round(str2num(get(hObject,'String')));
if (p<1 || p>8) p=1; set(hObject,'String','1');end
handles.Processes=p;
guidata(hObject, handles);
% hObject    handle to JobTag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of JobTag as text
%        str2double(get(hObject,'String')) returns contents of JobTag as a double


% --- Executes during object creation, after setting all properties.
function JobTag_CreateFcn(hObject, eventdata, handles)
% hObject    handle to JobTag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
